...
func TestClient_New(t *testing.T) {
	...
	if _, _, err := client.New(t.Context(), fn.Function{Root: root, Runtime: TestRuntime, Namespace: TestNamespace}); err != nil {
		t.Fatal(err)
	}
}
...
func TestClient_New_RuntimeRequired(t *testing.T) {
	...
	_, _, err := client.New(t.Context(), fn.Function{Root: root})
	if err == nil {
...
func TestClient_New_ExtantAborts(t *testing.T) {
	...
	if _, _, err := client.New(t.Context(), fn.Function{Root: root}); err == nil {
		t.Fatal("error expected initializing a path already containing an initialized function")
	}
}
...
func TestClient_New_NonemptyAborts(t *testing.T) {
	...
	if _, _, err := client.New(t.Context(), fn.Function{Root: root}); err == nil {
		t.Fatal("error expected initializing a function in a nonempty directory")
	}
}
...
func TestClient_New_RuntimeNotFoundError(t *testing.T) {
	...
	_, _, err := client.New(t.Context(), fn.Function{Root: root, Runtime: "invalid"})
...
// TestClient_New_ImageRegistryDefaults ensures that a Registry which does not have
...
func TestClient_Update(t *testing.T) {
	...
	var f fn.Function
	var err error
	if _, f, err = client.New(t.Context(), fn.Function{Runtime: TestRuntime, Root: root}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = client.Deploy(t.Context(), f); err != nil {
		t.Fatal(err)
	}
...
// 'default' gets overridden by 'already deployed' if applicable and all gets
...
func TestClient_Remove_ByPath(t *testing.T) {
	...
	var f fn.Function
	var err error
	if _, f, err = client.New(t.Context(), fn.Function{Runtime: TestRuntime, Root: root, Namespace: namespace}); err != nil {
		t.Fatal(err)
	}
...
func TestClient_Pipelines_Deploy_Image(t *testing.T) {
	...
	if _, f, err = client.RunPipeline(t.Context(), f); err != nil {
		t.Fatal(err)
	}
	expected := "example.com/alice/myfunc:latest"
...
func TestClient_Deploy_UnbuiltErrors(t *testing.T) {
	...
	_, err = client.Deploy(t.Context(), f)
	if err == nil {
...
func TestClient_New_BuildersPersisted(t *testing.T) {
	...
	var f1 fn.Function
	var err error
	if _, f1, err = client.New(t.Context(), f0); err != nil {
		t.Fatal(err)
	}
...
func TestClient_Scaffold(t *testing.T) {
	...
	if err := client.Scaffold(t.Context(), f, filepath.Join(root, out)); err != nil {
		t.Fatal(err)
	}
...
func TestClient_Invoke_HTTP(t *testing.T) {
	...
	h, r, err := client.Invoke(t.Context(), f.Root, "", message)
	if err != nil {
		t.Fatal(err)
	}
...
func TestClient_Invoke_CloudEvent(t *testing.T) {
	...
	// Invoke the function, which will use the mock Runner
	_, r, err := client.Invoke(t.Context(), f.Root, "", message)
	if err != nil {
		t.Fatal(err)
	}
...
func TestClient_Instances(t *testing.T) {
	...
	instance, err := client.Instances().Local(t.Context(), f)
	if err != nil {
		t.Fatal(err)
	}
